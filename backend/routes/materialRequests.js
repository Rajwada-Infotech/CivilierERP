...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (!["Draft", "Rejected", "Approved"].includes(check.recordset[0].Status))
      return res.status(409).json({
        error: `Cannot delete a Material Request with status "${check.recordset[0].Status}". Only Draft, Rejected, or Approved requests can be deleted.`,
      });
