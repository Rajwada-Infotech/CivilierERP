...
const OCCC_SELECT = `
  SELECT oc.*, cu.name AS CreatedByName, uu.name AS UpdatedByName
...
router.get("/", requirePageRight("crm-oc-cc", "view"), async (req, res) => {
  try {
    ...
    const result = await req0.query(`${OCCC_SELECT} ${where.length ? "WHERE " + where.join(" AND ") : ""} ORDER BY oc.CreatedAt DESC`);
