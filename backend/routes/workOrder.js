...
router.post("/", async (req, res) => {
  const {
    ...
    ContractorId, TotalAmount, Remarks, TermsAndConditions, CreatedBy
  } = req.body
  try {
    const pool = getPool()
...
router.put("/:id", async (req, res) => {
  const {
    CompanyId, ProjectId, DocumentNumber, DocumentDate,
    ContractorId, TotalAmount, Remarks, TermsAndConditions, UpdatedBy
  } = req.body
  try {
    const pool = getPool()
    await pool.request()
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    await pool.request()
      .input("WorkOrderHeaderId", sql.Int, req.params.id)
      .query("DELETE FROM dbo.WorkOrderActivities WHERE WorkOrderHeaderId = @WorkOrderHeaderId")
    await pool.request()
      .input("Id", sql.Int, req.params.id)
...
router.post("/:id/activities", async (req, res) => {
  const {
    ActivityGroupId, ActivityId, UOMId,
...
router.delete("/:id/activities/:activityId", async (req, res) => {
  try {
    ...
    await pool.request()
      ...
      .query("DELETE FROM dbo.WorkOrderActivityMaterials WHERE WorkOrderActivityId = @WorkOrderActivityId")
    await pool.request()
      .input("Id", sql.Int, req.params.activityId)
...
router.post("/:id/save-full", async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .input("TermsAndConditions", sql.NVarChar,       header.TermsAndConditions || null)
    ...
    if (keptActivityIds.length > 0) {
      await pool.request()
        ...
        .query(`
          ...
          INNER JOIN dbo.WorkOrderActivities a ON a.Id = m.WorkOrderActivityId
          WHERE a.WorkOrderHeaderId = @WorkOrderHeaderId
          AND a.Id NOT IN (${keptActivityIds.join(",")})
        `)
      await pool.request()
        .input("WorkOrderHeaderId", sql.Int, headerId)
        .query(`
          ...
          AND Id NOT IN (${keptActivityIds.join(",")})
        `)
    } else {
    ...
    res.json({
      ...
      activityCount: keptActivityIds.length
    })
  } catch (err) {
    res.status(500).json({ error: err.message })
  }
