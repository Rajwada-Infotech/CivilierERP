...
router.get("/", cache("new-payment", 300), async (req, res) => {
  try {
    ...
    const docNumber = req.query.docNumber
      ? String(req.query.docNumber).trim()
      : "";
...
router.post(
  "/deduct-cheque",
  requirePageRight("new-payment", "edit"),
  async (req, res) => {
    const { lotId, chequeNo } = req.body;
    if (!lotId) return res.status(400).json({ error: "lotId is required" });
    if (!chequeNo)
      return res.status(400).json({ error: "chequeNo is required" });
    ...
    `);
      ...
      if (num < lot.ChequeStartNumber || num > lot.ChequeEndNumber) {
        return res
          .status(400)
          .json({ error: "Cheque number out of lot range" });
      }
...
router.post(
  "/",
  requirePageRight("new-payment", "create"),
    ...
    try {
      ...
      if (PExpenseRef) {
        ...
        if (isEmiRef) {
          ...
          ebCheck = await pool
            ...
            .query(
              "SELECT EStatus FROM dbo.ExpenseBooking WHERE EDocNo = @EDocNo",
            );
        }
      ...
      const insertResult = await pool
        ...
        .input(
          "PAmount",
          sql.Decimal(18, 2),
          PAmount != null ? Number(PAmount) : null,
        )
      ...
      if (
        ...
        return res.status(409).json({
          error:
            "Cheque number already used in another payment. Pick a different cheque.",
        });
      }
...
router.delete(
  "/:id",
  requirePageRight("new-payment", "delete"),
  async (req, res) => {
    const { id } = req.params;
    try {
      const pool = getPool();
      ...
      const brsCheck = await pool.request().input("PPaymentID", sql.Int, id)
        .query(`
    ...
    `);
      if (Number(brsCheck.recordset[0]?.cnt) > 0) {
        return res.status(409).json({
          error: "brs_matched",
          message:
            "This payment is matched in the Bank Reconciliation Statement. Unmatch/reverse it in the BRS before deleting.",
        });
      }

      const refRow = await pool
        .request()
        .input("PPaymentID", sql.Int, id)
        .query(
    ...
    try {
      ...
      const message =
        result.newStatus === "Approved"
          ? "Payment approved"
          : `Level ${result.level} of ${result.totalLevels} approved — awaiting further approval`;
