...
router.post("/", async (req, res) => {
  const { tenant_id, name, domain, admin_email, plan, max_users, db_name, server } = req.body;
  try {
    const pool = getPool();
    await pool
      .request()
...
router.put("/:id", async (req, res) => {
  const { id } = req.params;
  const { tenant_id, name, domain, admin_email, plan, max_users, db_name, server, status } = req.body;
  try {
    const pool = getPool();
    await pool
      .request()
