...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await pool.request().query("SELECT * FROM dbo.ExpenseBooking")
...
router.post("/", async (req, res) => {
  const {
    ...
    EDocNo, EEmiPayment, EReminder, ERemarks,
    EStatus, ECompanyName
  } = req.body
  try {
    const pool = await sql.connect()
    await pool.request()
      ...
      .input("ECompanyName",  sql.VarChar,        ECompanyName || null)
      .query(`
