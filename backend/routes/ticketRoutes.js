...
// Order: validate filename → DB access check → file-exists check → sendFile
// This way auth/permission errors are returned correctly even for migrated
// tickets whose physical files may no longer exist on disk.
async function serveTicketFile(req, res) {
  try {
    ...
    const ticketResult = await pool
      .request()
      .input("fileUrl", sql.NVarChar(512), filePattern).query(`
        SELECT TOP 1 id
        FROM dbo.tickets
    ...
    // Check access before revealing whether the file exists on disk
    const access = await getAccessibleTicket(pool, ticketId, actor);
    const accessError = sendTicketAccessError(res, access);
    if (accessError) return accessError;

    // Now check physical file existence (may be missing for migrated/old tickets)
    if (!fs.existsSync(filePath)) {
      return res.status(410).json({ error: "Attachment no longer available" });
    }
  ...
  try {
    ...
    const upload = multer({
      ...
      fileFilter: (_req, file, cb) => {
        ...
        if (
          ...
          cb(
            new Error("Only images (jpg, png, gif, webp) and PDFs are allowed"),
          );
        }
      },
    });
    ...
    router.get("/", async (req, res) => {
      try {
        ...
        const statusIn = hasStatusFilter
          ...
          : null;
        const whereClause = statusIn ? `WHERE t.status IN (${statusIn})` : "";
        const countWhereClause = statusIn
        ...
        const countResult = await pool
          .request()
          .query(
            `SELECT COUNT(*) AS total FROM dbo.tickets ${countWhereClause}`,
          );
        const total = countResult.recordset[0]?.total ?? 0;
    ...
    router.get("/:id", async (req, res) => {
      try {
        ...
        // Fetch DB attachments for this ticket (ticket-level, comment_id IS NULL)
        const attachResult = await pool.request().input("tid", sql.Int, id)
          .query(`
        ...
        const visibleComments = isTicketAdmin(actor.role)
          ? comments.recordset
          : comments.recordset.filter(
              (comment) => !Number(comment.is_internal),
            );
        ...
        // Build attachment URLs from DB records
    ...
    router.post("/upload", upload.array("file", 20), async (req, res) => {
      try {
        ...
        res.json({
          success: true,
          // Legacy single-URL compat (frontend reads data.url for first file)
          url: results[0]?.url,
    ...
    router.get("/attachment/:id", async (req, res) => {
      try {
        ...
        // Check ticket access
        const access = await getAccessibleTicket(
          pool,
    ...
    // Keep for backward compat with any existing comment markers that still use
    // the old /api/tickets/file/<filename> pattern. Returns 410 if file is gone.
    router.get("/file/:filename", async (req, res) => {
      try {
        const actor = requireActor(req, res);
        if (!actor) return;
        // All old files are gone — return 410 Gone so the frontend shows a graceful
    ...
    async function createTicketHandler(req, res) {
      try {
        ...
        const {
          ...
          company_id,
          project_id,
          attachment_path, // legacy field — kept for compat, ignored if attachmentIds present
          attachmentIds, // array of attachment IDs pre-uploaded via /upload
        } = req.body;
        ...
        // If client pre-uploaded attachments, link them to this ticket
        if (
          newTicketId &&
      ...
      try {
        ...
        if (result.rowsAffected?.[0] === 0) {
          const current = await pool
            ...
            .input("id", sql.Int, id)
            .query(
              "SELECT status, assigned_to FROM dbo.tickets WHERE id = @id",
            );
          if (!current.recordset.length) {
            return res.status(404).json({ error: "Ticket not found" });
          }
    ...
    router.post("/comment/:id", async (req, res) => {
      try {
        ...
        if (
          ...
          return res
            .status(400)
            .json({ error: "Comment or attachment required" });
