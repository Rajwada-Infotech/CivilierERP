...
router.post("/", requirePageRight("partner-master", "create"), async (req, res) => {
  ...
  try {
    ...
    // Suffix wording is "(for investment)" / "(for withdrawing)" — plain-
    ...
    // reads differently.
    const capitalHeadId = await insertHead(`${baseCode}${CAP_SUFFIX}`, groups.capital.id, `${name} (for investment)`);
    const currentHeadId = await insertHead(`${baseCode}${CUR_SUFFIX}`, groups.current.id, `${name} (for withdrawing)`);
    await tx.commit();
...
router.put("/:code", requirePageRight("partner-master", "edit"), async (req, res) => {
  ...
  try {
    ...
    const request = pool
      ...
      .input("CapDisplayName", sql.NVarChar(200), newName ? `${newName} (for investment)` : null)
      .input("CurDisplayName", sql.NVarChar(200), newName ? `${newName} (for withdrawing)` : null)
      .input(
        "LHeadStatus",
        sql.Bit,
