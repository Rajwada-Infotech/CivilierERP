...
// POST / — create
router.post("/", authMiddleware, requirePageRight("return-reason-master", "create"), async (req, res) => {
  ...
  try {
    ...
    const dup = await pool.request()
      ...
      .query(`SELECT ReasonId FROM dbo.ReturnReasonMaster WHERE ReasonName = @name`);
...
// PUT /:id — update
router.put("/:id", authMiddleware, requirePageRight("return-reason-master", "edit"), async (req, res) => {
  ...
  try {
    ...
    const existing = await pool.request().input("id", sql.Int, id)
      .query(`SELECT ReasonId FROM dbo.ReturnReasonMaster WHERE ReasonId = @id`);
    if (!existing.recordset.length) return res.status(404).json({ error: "Reason not found" });

    const dup = await pool.request()
      .input("name", sql.NVarChar(200), rName)
      .input("id",   sql.Int,           id)
      .query(`SELECT ReasonId FROM dbo.ReturnReasonMaster WHERE ReasonName = @name AND ReasonId != @id`);
    if (dup.recordset.length > 0)
    ...
    await pool.request()
      ...
      .query(`
        UPDATE dbo.ReturnReasonMaster SET
...
router.delete("/:id", authMiddleware, requirePageRight("return-reason-master", "delete"), async (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) return res.status(400).json({ error: "Invalid ID" });
  const actor = req.user?.email || req.user?.name || "system";
  ...
  try {
    ...
    await pool.request()
      ...
      .input("updatedBy", sql.NVarChar(200), actor)
      .query(`
        UPDATE dbo.ReturnReasonMaster SET IsActive = 0, UpdatedBy = @updatedBy, UpdatedAt = SYSDATETIME()
        WHERE ReasonId = @id
      `);
    res.json({ success: true });
  } catch (err) {
