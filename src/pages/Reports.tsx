...
const ALL_REPORTS: ReportDef[] = [
  ...
  {
    id: "fixed-asset-register-report",
    label: "Fixed Asset Register",
    description: "All registered fixed assets with cost, category & custody",
    icon: Archive,
    color: "#8b5cf6",
    apiPath: "/api/fixed-assets",
    // GET / on fixedAssets.js filters on PurchaseDate; supports companyId/finYear.
    filterConfig: {
      companyParam: "companyId",
      finYearParam: "finYear",
      singleDateParam: null,
    ...
    columns: [
      ...
      { header: "Asset Name", accessor: (r) => (r.AssetName ?? "—") as string },
      { header: "FA Item Code", accessor: (r) => (r.FAItemCode ?? "—") as string },
      { header: "Category", accessor: "AssetCategory" },
      { header: "Asset Code", accessor: (r) => (r.AssetCode ?? "—") as string },
      { header: "Purchase Date", accessor: (r) => (r.PurchaseDate ? String(r.PurchaseDate).slice(0, 10) : "—") },
      ...
      { header: "Custodian", accessor: (r) => (r.Custodian ?? "—") as string },
      { header: "Depreciation Type", accessor: (r) => (r.DepreciationType ?? "—") as string },
      { header: "Depreciation Rate", accessor: (r) => (r.DepreciationRate != null ? `${r.DepreciationRate}%` : "—") },
      { header: "Status", accessor: "AssetStatus" },
    ],
  },
  {
    id: "fa-inventory-report",
    label: "FA Inventory Report",
    description: "Fixed-asset tagging entries — FA Item Codes generated per batch",
    icon: Tag,
    color: "#0d9488",
    apiPath: "/api/fixed-asset-tagging",
    // GET / on fixedAssetTagging.js filters on DocDate; supports companyId/finYear.
    filterConfig: {
      companyParam: "companyId",
      finYearParam: "finYear",
      singleDateParam: null,
    ...
    columns: [
      ...
      { header: "Asset / Item", accessor: (r) => (r.AssetName ?? "—") as string },
      { header: "FA Item Code", accessor: (r) => (r.FAItemCode ?? "—") as string },
      { header: "Tagged Qty", accessor: (r) => String(r.TaggedQty ?? "—") },
      { header: "Status", accessor: "Status" },
      { header: "Record Status", accessor: (r) => (r.RecordStatus ?? "—") as string },
      { header: "Remarks", accessor: (r) => (r.Remarks ?? "—") as string },
    ],
  },
  {
    id: "depreciation-setup-report",
    label: "Depreciation Setup Report",
    description: "Depreciation rate & method configured per asset category",
    icon: TrendingDown,
    color: "#f97316",
    apiPath: "/api/depreciation-setup",
    // GET / on depreciationSetup.js is a plain category master — no company/date scoping.
    filterConfig: {
      companyParam: null,
      finYearParam: null,
      singleDateParam: null,
    ...
    columns: [
      ...
      { header: "Depreciation Type", accessor: "DepreciationType" },
      { header: "Depreciation Rate", accessor: (r) => (r.DepreciationRate != null ? `${r.DepreciationRate}%` : "—") },
      { header: "Effective From", accessor: (r) => (r.EffectiveFrom ? String(r.EffectiveFrom).slice(0, 10) : "—") },
      { header: "Status", accessor: "Status" },
    ],
  },
  {
    ...
    description: "Task completion, delays & follow-up counts per assignee",
    icon: CalendarClock,
    color: "#2dd4bf",
    apiPath: "/api/task-performance-report",
    // GET / filters on CreatedAt; no finYear param on this route.
    filterConfig: {
      companyParam: "companyId",
      singleDateParam: null,
    ...
    columns: [
      { header: "Task No", accessor: (r) => (r.TaskNo ?? `#${r.Id}`) as string },
      { header: "Case No", accessor: (r) => (r.CaseNumber ?? "—") as string },
      { header: "Company", accessor: "CompanyName" },
      { header: "Project", accessor: "ProjectName" },
      { header: "Subject", accessor: "Subject" },
      { header: "Created By", accessor: (r) => (r.CreatedByName ?? "—") as string },
      { header: "Assignee", accessor: (r) => (r.FollowerName ?? "—") as string },
      { header: "Start Date", accessor: (r) => (r.TaskStartDate ? String(r.TaskStartDate).slice(0, 10) : "—") },
      { header: "Due Date", accessor: (r) => (r.TaskDueDate ? String(r.TaskDueDate).slice(0, 10) : "—") },
      { header: "Completion Date", accessor: (r) => (r.TaskCompletionDate ? String(r.TaskCompletionDate).slice(0, 10) : "—") },
      { header: "Completion Days", accessor: (r) => String(r.CompletionDayCount ?? "—") },
      { header: "Follow-Ups", accessor: (r) => String(r.FollowUpAttendCount ?? 0) },
      { header: "Progress %", accessor: (r) => `${r.EffectiveProgress ?? r.Progress ?? 0}%` },
      { header: "Status", accessor: "Status" },
      { header: "Priority", accessor: "Priority" },
      { header: "Delay Days", accessor: (r) => String(r.DelayDays ?? "—") },
    ],
  },
  {
    id: "entry-type-doc-followup-report",
    label: "Entry Type & Document Follow-Up Report",
    description: "Follow-up activity grouped by entry type and linked document",
    icon: ListTree,
    color: "#c026d3",
    apiPath: "/api/entry-type-doc-followup-report",
    // GET / filters on the follow-up's own CreatedAt; no finYear param on this route.
    filterConfig: {
      companyParam: "companyId",
      singleDateParam: null,
      dateFromParam: "startDate",
    ...
    columns: [
      { header: "Entry Type", accessor: (r) => (r.EntryTypeLabel ?? "Unassigned") as string },
      { header: "Document", accessor: (r) => (r.DocumentLabel ?? "—") as string },
      { header: "Document ID", accessor: (r) => (r.DocumentId ?? "—") as string },
      { header: "Task ID", accessor: (r) => String(r.TaskId ?? "—") },
      { header: "Company", accessor: (r) => (r.CompanyName ?? "—") as string },
      { header: "Project", accessor: (r) => (r.ProjectName ?? "—") as string },
      { header: "Follow-Up Date", accessor: (r) => (r.FollowUpDate ? String(r.FollowUpDate).slice(0, 16).replace("T", " ") : "—") },
      { header: "Follow-Up User", accessor: (r) => (r.FollowUpUserName ?? "—") as string },
      { header: "Status", accessor: (r) => (r.TaskStatus ?? "—") as string },
      { header: "Priority", accessor: (r) => (r.Priority ?? "—") as string },
      { header: "Follow-Up Attend Count", accessor: (r) => String(r.FollowUpAttendCount ?? 0) },
    ],
  },
  {
    id: "po-register",
