...
import {
  FileText, Landmark, Plus, Edit2, Trash2,
  RotateCcw, Check, X, Search, Hash, Calculator, BookOpen,
} from 'lucide-react'
...
const addCheque     = (data: object) => fetch(BASE, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json())
const updateCheque  = (id: string, data: object) => fetch(`${BASE}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json())
const deleteCheque  = (id: string) => fetch(`${BASE}/${id}`, { method: 'DELETE' }).then(r => r.json())
...
const EMPTY: FormState = {
  bankId: '', bankName: '', accountNumber: '', ifscCode: '',
  lotNumber: '', chqStart: '', chqEnd: '',
  totalCheques: 0, remarks: '', status: true,
}
...
const ChequeMaster: React.FC = () => {
  ...
  const { data: chequeData, isLoading: loadingCheques } = useQuery({ queryKey: ['cheques'], queryFn: getCheques })
  const { data: bankData,   isLoading: loadingBanks   } = useQuery({ queryKey: ['banks'],   queryFn: getBanks   })
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select value={form.bankId} onChange={e => handleBankChange(e.target.value)} className={`${inp} pl-8 ${errors.bankId ? 'border-destructive' : ''}`}>
                    ...
                    {dbBanks.map(b => <option key={b.BId} value={String(b.BId)}>{b.BName}{b.BBranch ? ` — ${b.BBranch}` : ''}</option>)}
                  </select>
                </div>
              ...
              <div>
                <label className="block text-[11px] uppercase tracking-widest font-heading text-muted-foreground mb-1.5">
                  ...
                  <span className="ml-2 normal-case text-[10px] text-muted-foreground/60">(auto-filled)</span>
                </label>
                <div className="relative">
              ...
              <div>
                <label className="block text-[11px] uppercase tracking-widest font-heading text-muted-foreground mb-1.5">
                  IFSC Code <span className="ml-2 normal-case text-[10px] text-muted-foreground/60">(auto-filled)</span>
                </label>
                <div className="relative">
                  <Hash size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
          ...
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border bg-card/60">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">
                ...
                {filtered.length > 0 && <span className="ml-2 text-primary font-semibold">· {filtered.reduce((s, r) => s + (r.TotalCheques || 0), 0).toLocaleString()} total cheques</span>}
              </p>
            </div>
            <div className="relative">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  {['Bank', 'Account No.', 'IFSC', 'Lot No.', 'Chq Start', 'Chq End', 'Total', 'Status', 'Actions'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-[10px] font-heading uppercase tracking-widest text-muted-foreground whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {filtered.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-10 text-center text-muted-foreground text-sm">{search ? 'No cheque lots match your search.' : 'No cheque lots yet. Add one above.'}</td></tr>
                ) : filtered.map(row => {
                  const bank = dbBanks.find(b => b.BId === row.BankId)
                  const id   = String(row.CId)
                  return (
                    <tr key={id} className={`hover:bg-muted/20 transition-colors ${editingId === id ? 'bg-primary/5 border-l-2 border-l-primary' : ''}`}>
                      <td className="px-4 py-3 text-foreground font-body">{bank?.BName || '—'}</td>
                      <td className="px-4 py-3 font-mono text-foreground text-xs">{row.AccountNumber ? <span className="bg-muted px-2 py-0.5 rounded">{row.AccountNumber}</span> : '—'}</td>
                      <td className="px-4 py-3 font-mono text-foreground text-xs">{row.IFSCCode ? <span className="bg-muted px-2 py-0.5 rounded">{row.IFSCCode}</span> : '—'}</td>
                      <td className="px-4 py-3 text-foreground font-heading font-medium">{row.ChequeLotNumber}</td>
                      <td className="px-4 py-3 font-mono text-foreground">{row.ChequeStartNumber}</td>
                      <td className="px-4 py-3 font-mono text-foreground">{row.ChequeEndNumber}</td>
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-primary/10 border border-primary/20 text-primary font-heading font-semibold text-sm">
                          <Calculator size={11} />{(row.TotalCheques || 0).toLocaleString()}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {deleteId === id ? (
                            <>
                              ...
                              <button onClick={() => handleDelete(id)} className="p-1.5 rounded-lg text-destructive hover:bg-destructive/10 transition-colors"><Check size={13} /></button>
                            ...
                            <>
                              <button onClick={() => handleEdit(row)} className="p-1.5 rounded-lg text-primary hover:bg-primary/10 transition-colors"><Edit2 size={13} /></button>
                              <button onClick={() => setDeleteId(id)} className="p-1.5 rounded-lg text-destructive hover:bg-destructive/10 transition-colors"><Trash2 size={13} /></button>
                            </>
                          )}
                        </div>
                      </td>
