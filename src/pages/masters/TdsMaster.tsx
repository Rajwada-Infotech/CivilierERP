...
const updateTds = (id: string, data: object) => fetch(`${BASE}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json())
const deleteTds = (id: string) => fetch(`${BASE}/${id}`, { method: 'DELETE' }).then(r => r.json())
...
const TdsMaster: React.FC = () => {
  ...
  const columnRenderers: Record<string, (value: unknown) => React.ReactNode> = {
    ...
    percentage: (value) => (
      <span className="font-mono text-sm">{value !== '' ? `${value}%` : '—'}</span>
    ),
    nature: (value) => (
  ...
  if (error)     return <div className="p-6 text-red-500">Failed to load TDS records.</div>
