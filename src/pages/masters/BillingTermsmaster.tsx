...
const updateBillingTerm = (id: string, data: object) => fetch(`${BASE}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json())
const deleteBillingTerm = (id: string) => fetch(`${BASE}/${id}`, { method: 'DELETE' }).then(r => r.json())
...
const BillingTermsMaster: React.FC = () => {
  ...
  if (error)     return <div className="p-6 text-red-500">Failed to load billing terms.</div>
    ...
    <>
      ...
      <MasterPage
        ...
        fields={[
          ...
          { name: 'gst',         label: 'GST',         type: 'select',   required: true, options: ['Before', 'After'] },
          { name: 'type',        label: 'Type',        type: 'select',   required: true, options: ['Increase', 'Decrease'] },
