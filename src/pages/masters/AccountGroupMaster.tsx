...
// ─── VS Code-style Group Tree Picker ─────────────────────────────────────────
...
function GroupTreePicker({
  ...
  // Expand all roots by default when panel opens
  useEffect(() => {
    if (open) {
  ...
  // Preview pane: show the hovered or selected group's details
  const previewId = hoverId || value || null;
  const previewGroup = useMemo(
    () => allGroups.find((g) => g._id === previewId) ?? null,
    [previewId, allGroups],
  );
  const previewChildren = useMemo(
    () => allGroups.filter((g) => g.parentId === previewId),
    [allGroups, previewId],
  );
  const previewPath = useMemo(
    () => (previewGroup ? getBelongsTo(previewGroup._id, allGroups) : null),
    [previewGroup, allGroups],
  );

  // GL accounts for the previewed group
  ...
  const selectedName = useMemo(() => {
    ...
    return allGroups.find((g) => g._id === value)?.name ?? null;
  }, [value, allGroups]);
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-lg border text-sm transition-colors text-left
          ${open ? "border-primary/50 ring-2 ring-primary/20 bg-background" : "border-border bg-background hover:border-primary/30"}
        `}
      >
        <span className={`flex items-center gap-2 min-w-0 ${selectedName ? "text-foreground" : "text-muted-foreground/50"}`}>
          {selectedName ? (
            ...
            <>
              <Layers size={13} className="text-muted-foreground/40 shrink-0" />
              — Top-level group (no parent)
            </>
          )}
        </span>
        <ChevronsUpDown size={13} className="text-muted-foreground/50 shrink-0" />
      </button>

      {/* ── Sliding panel ── */}
      {open && (
        <div className="absolute left-0 top-[calc(100%+6px)] z-50 w-full rounded-xl border border-border bg-card shadow-2xl overflow-hidden flex flex-col"
          style={{ maxHeight: 420 }}>
          ...
          <div className="flex items-center gap-2 px-3 py-2.5 border-b border-border bg-muted/20 shrink-0">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full text-xs pl-7 pr-3 py-1.5 rounded-md bg-background border border-border text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-primary/30"
              />
              {pickerSearch && (
          ...
          <div className="flex min-h-0 flex-1">
            {/* ── Tree / search results ── */}
            <div className="flex-1 overflow-y-auto py-1.5 min-w-0 border-r border-border/50">
              {/* No-parent option */}
              <div
                className={`flex items-center gap-1.5 px-3 py-1 mx-1 rounded text-xs cursor-pointer transition-colors mb-0.5 ${
                  value === ""
                    ? "bg-primary/15 text-primary font-semibold"
              ...
              {searchResults ? (
                searchResults.length === 0 ? (
                  ...
                  searchResults.map((g) => (
                    <div
                      key={g._id}
                      className={`flex items-center gap-2 px-3 py-1 mx-1 rounded text-xs cursor-pointer transition-colors ${
                    ...
                    >
                      ...
                      <span className="font-mono text-[9px] text-muted-foreground/50 ml-auto shrink-0">{g.code}</span>
                    </div>
                  ))
                )
              ) : (
                tree.map((node) => (
                  <div
                    key={node._id}
                    onMouseEnter={() => setHoverId(node._id)}
                    onMouseLeave={() => setHoverId(null)}
                  >
                    <TreePickerNode
                      node={node}
            ...
            {/* ── Detail / linked entities panel ── */}
            {previewGroup && (
              <div className="w-52 shrink-0 overflow-y-auto py-3 px-3 space-y-3 bg-muted/5">
                {/* Group info */}
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-muted-foreground font-heading mb-1">Group</p>
                  <p className="text-xs font-semibold text-foreground leading-snug">{previewGroup.name}</p>
                  <p className="font-mono text-[10px] text-primary/70 mt-0.5">{previewGroup.code}</p>
                  {previewPath && (
                    <p className="text-[10px] text-muted-foreground mt-1 leading-snug">{previewPath}</p>
                  )}
                </div>
                ...
                {previewChildren.length > 0 && (
                  <div>
                    <p className="text-[9px] uppercase tracking-widest text-muted-foreground font-heading mb-1.5 flex items-center gap-1">
                      <FolderOpen size={9} /> Sub-groups ({previewChildren.length})
                    </p>
                    <div className="space-y-0.5">
                      {previewChildren.map((c) => (
                        <div key={c._id} className="flex items-center gap-1.5 text-[10px] text-foreground/70 py-0.5">
                          <Folder size={9} className="text-muted-foreground/40 shrink-0" />
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-muted-foreground font-heading mb-1.5 flex items-center gap-1">
                    <BookOpen size={9} /> Ledger accounts
                  </p>
                  {glLoading ? (
                    <p className="text-[10px] text-muted-foreground/50">Loading…</p>
                  ) : !glAccounts || (glAccounts as any[]).length === 0 ? (
                    <p className="text-[10px] text-muted-foreground/40 italic">None linked</p>
                  ) : (
                    <div className="space-y-0.5">
                      {(glAccounts as any[]).slice(0, 8).map((a: any) => (
                        <div key={a.LHeadId ?? a._id} className="flex items-center gap-1.5 text-[10px] text-foreground/70 py-0.5">
                          <BookOpen size={9} className="text-primary/30 shrink-0" />
                          <span className="truncate">{a.LHeadName ?? a.name}</span>
                        </div>
                      ))}
                      {(glAccounts as any[]).length > 8 && (
                        <p className="text-[9px] text-muted-foreground/40 mt-1">+{(glAccounts as any[]).length - 8} more</p>
                      )}
                    </div>
                  )}
                </div>
