...
// ─── API ──────────────────────────────────────────────────────────────────────
const BASE = 'http://localhost:5000/api/hsn'

const getHsn    = () => fetch(BASE).then(r => r.json())
const addHsn    = (data: object) => fetch(BASE, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json())
const updateHsn = (code: string, data: object) => fetch(`${BASE}/${code}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json())
const deleteHsn = (code: string) => fetch(`${BASE}/${code}`, { method: 'DELETE' }).then(r => r.json())
...
const HsnMaster: React.FC = () => {
  ...
  const columnRenderers: Record<string, (value: unknown) => React.ReactNode> = {
    ...
    igstRate: (value) => value !== '' ? <span className="font-mono text-sm">{Number(value)}%</span> : '—',
    cgstRate: (value) => value !== '' ? <span className="font-mono text-sm">{Number(value)}%</span> : '—',
    sgstRate: (value) => value !== '' ? <span className="font-mono text-sm">{Number(value)}%</span> : '—',
  }

  if (isLoading) return <div className="p-6 text-muted-foreground">Loading...</div>
  if (error)     return <div className="p-6 text-red-500">Failed to load HSN records.</div>
