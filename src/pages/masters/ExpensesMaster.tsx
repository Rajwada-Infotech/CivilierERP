...
import { Pencil, Trash2, X, Check, RotateCcw, Plus, BookOpen, Tag, ChevronDown, AlertCircle } from "lucide-react";
import { DataTable, type ColumnDef } from "@/components/ui/DataTable";

interface AccountGroupData { AGId: number; Name: string; }
interface AccountGroup { _id: string; name: string; }
interface LedgerHead { LHeadId: number; LHeadName: string; LHeadType: string | null; LBelongsTo: number | null; GroupName: string | null; LHeadStatus: boolean; }
interface LedgerForm { LHeadName: string; LHeadType: string; LBelongsTo: string; }

const EMPTY_FORM: LedgerForm = { LHeadName: "", LHeadType: "", LBelongsTo: "" };
...
const ExpensesMaster: React.FC = () => {
  ...
  const { data: groupsData, isLoading: groupsLoading } = useQuery({ queryKey: ["account-groups"], queryFn: getAccountGroups, retry: 1, staleTime: 5 * 60 * 1000 });
  const { data: ledgersData, isLoading: ledgersLoading, isError: ledgersError } = useQuery({ queryKey: ["ledger-heads"], queryFn: () => getList(), retry: 1, staleTime: 5 * 60 * 1000 });

  const accountGroups: AccountGroup[] = useMemo(() => {
    if (!Array.isArray(groupsData)) return [];
    return (groupsData as AccountGroupData[]).filter((item) => item.AGId != null && item.Name).map((item) => ({ _id: String(item.AGId), name: item.Name }));
  }, [groupsData]);

  const ledgers: LedgerHead[] = useMemo(() => (Array.isArray(ledgersData) ? ledgersData : []), [ledgersData]);
  ...
  const createMut = useMutation({
    mutationFn: (data: LedgerForm) => addRecord({ LHeadName: data.LHeadName.trim(), LBelongsTo: data.LBelongsTo ? Number(data.LBelongsTo) : null }, data.LHeadType || "Ledger"),
    onSuccess: () => { toast.success("Ledger account created"); invalidate(); resetForm(); },
    onError: (e: Error) => toast.error(e.message),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: LedgerForm }) => updateRecord(id, { LHeadName: data.LHeadName.trim(), LBelongsTo: data.LBelongsTo ? Number(data.LBelongsTo) : null }, data.LHeadType || "Ledger"),
  ...
  const startEdit = (l: LedgerHead) => {
    ...
    setForm({ LHeadName: l.LHeadName ?? "", LHeadType: l.LHeadType ?? "", LBelongsTo: l.LBelongsTo != null ? String(l.LBelongsTo) : "" });
    setErrors({});
  ...
  return (
    <>
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Define ledger accounts with a name, type and account group</p>
      </div>
        ...
        <div className="p-5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-5">
            <div>
              ...
              <input value={form.LHeadName} onChange={(e) => { setForm((p) => ({ ...p, LHeadName: e.target.value })); setErrors((p) => ({ ...p, LHeadName: false })); }} placeholder="e.g. Cash in Hand" className={`w-full text-sm rounded-lg border px-3 py-2.5 bg-background text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/30 transition ${errors.LHeadName ? "border-red-400 ring-red-400" : "border-border"}`} />
            ...
            <div>
              ...
              <div className="relative">
                <select value={form.LHeadType} onChange={(e) => setForm((p) => ({ ...p, LHeadType: e.target.value }))} className="w-full text-sm rounded-lg border border-border px-3 py-2.5 bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition appearance-none">
      ...
      <div className="rounded-xl border border-border overflow-hidden bg-card">
        <DataTable
          ...
          emptyMessage={ledgers.length === 0 ? "No ledger accounts yet. Use the form above to create your first account." : "No results match your search."}
