...
const updateActivity   = (id: string, data: object) => fetch(`${BASE}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json())
const deleteActivity   = (id: string) => fetch(`${BASE}/${id}`, { method: 'DELETE' }).then(r => r.json())
...
const ActivityMaster: React.FC = () => {
  ...
  const groupOptions = groups.map(g => ({ value: String(g.id), label: g.activity_name }))
  ...
  const columnRenderers: Record<string, (value: unknown, row: RecordWithId) => React.ReactNode> = {
    activityType: (value) => {
      ...
      return (
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-heading border ${isGroup ? 'bg-violet-500/10 border-violet-500/20 text-violet-600' : 'bg-teal-500/10 border-teal-500/20 text-teal-600'}`}>
    ...
    shortDesc: (value) => <span className="font-mono text-xs tracking-wide text-muted-foreground">{String(value || '—')}</span>,
    groupName: (value, row) => {
      if (row.activityType !== 'Activity') return <span className="text-muted-foreground text-xs">—</span>
      return (
        <span className="inline-flex items-center gap-1 text-xs text-violet-600">
          <Layers size={11} />{String(value || '—')}
        </span>
      )
    },
  }
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <h1 className="text-xl font-heading font-bold text-foreground">Activity Master</h1>
      </div>
      <MasterPage
        title="Activity"
        fields={[
          { name: 'activityName', label: 'Activity Name',  type: 'text',   required: true },
          { name: 'shortDesc',    label: 'Short Desc',     type: 'text',   required: true },
          { name: 'activityType', label: 'Activity Type',  type: 'select', options: ['Group', 'Activity'], required: true, defaultValue: 'Group' },
          { name: 'groupId',      label: 'Belongs To Group', type: 'select', options: groupOptions.map(o => o.label) },
