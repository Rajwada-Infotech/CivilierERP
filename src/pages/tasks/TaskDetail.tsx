...
export default function TaskDetail() {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center px-4">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          {fetchError ?? "This task doesn't exist or you don't have access."}
        </p>
        <button
  ...
  const allCriteriaMet =
    ...
    task.qualityCriteria.every((q) => q.met);
    ...
    <>
      ...
      <div className="flex items-start justify-between gap-3 mb-6">
        <div className="flex items-start gap-2 min-w-0">
          <button
            onClick={() => navigate("/tasks")}
            className="mt-0.5 p-1.5 rounded-lg hover:bg-muted text-muted-foreground transition-colors shrink-0"
          >
            <ArrowLeft size={16} />
          </button>
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-heading ${cfg.bg} ${cfg.color}`}
              >
                {cfg.label}
              </span>
              <span
            ...
            <div className="rounded-xl bg-card border border-border p-4 sm:p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-muted-foreground font-heading">
                  {metCount}/{task.qualityCriteria.length} met
                </span>
              </div>
              <div className="w-full h-1.5 rounded-full bg-muted mb-4 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(metCount / task.qualityCriteria.length) * 100}%`,
              ...
              <div className="space-y-2">
                {task.qualityCriteria.map((qc) => (
                  <button
                    key={qc.id}
                    onClick={() =>
                      (isAssignee || isAdminOrAbove) &&
                      toggleQualityCriteria(task.id, qc.id)
                    }
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 border transition-colors ${qc.met ? "bg-green-500 border-green-500" : "border-border bg-muted"}`}
                    >
                      {qc.met && <Check size={11} className="text-white" />}
                    </div>
                    <span
                      className={`text-sm font-heading ${qc.met ? "text-green-400" : "text-foreground"}`}
                    >
                      {qc.label}
                    </span>
                  </button>
          ...
          <div className="rounded-xl bg-card border border-border p-4 sm:p-5">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg gradient-accent text-primary-foreground transition-all hover:-translate-y-0.5 shrink-0"
              >
                <Send size={14} />
              </button>
            </div>
            ...
            {[
              ...
              ...(task.closedAt
                ? [
                    {
                      label: "Closed At",
                      value: formatDateTime(task.closedAt!),
                      icon: CheckCircle2,
                    },
                  ]
                : []),
              ...
              ...(task.reviewedAt
                ? [
                    {
                      label: "Reviewed At",
                      value: formatDateTime(task.reviewedAt!),
                      icon: Clock,
                    },
                  ]
                : []),
            ...
            {isAssignee &&
              (task.status === "open" || task.status === "in_progress") && (
                ...
                >
                  ...
                  {allCriteriaMet
                    ? "Close Task"
                    : `${metCount}/${task.qualityCriteria.length} criteria met`}
                </button>
              )}
      ...
      <AnimatePresence>
        {showEdit && (
          <TaskFormModal editTask={task} onClose={() => setShowEdit(false)} />
        )}
      </AnimatePresence>
    </>
  );
