...
const PurchaseOrderMaster = () => {
  ...
  // Fetch Suppliers & Companies for dropdowns
  ...
  const mappedData: RecordWithId[] = dbItems.map((item) => ({
    _id: String(item.PurchaseOrderID ?? item.id ?? item._id ?? ""),
    poNumber: item.PurchaseOrderNo ?? item.PONumber ?? "",
    poDate: item.PODate ?? "",
    expectedDate: item.ExpectedDeliveryDate ?? item.ExpectedDate ?? "",
  ...
  const FIELDS: FieldDef[] = [
...

    {
      ...
      type: "select",
      required: true,
      options: suppliers.map((s: any) => s.SupplierName || s.name || s.Supplier || ""),
    },
    {
      name: "company",
      label: "Company",
      type: "select",
      required: true,
      options: companies.map((c: any) => c.CompanyName || c.name || c.Company || ""),
    },
  ...
  return (
    <>
      ...
      <MasterPage
        ...
        onFieldChange={handleFieldChange}     {/* Important for auto calculation */}
      />
    </>
  );
};
