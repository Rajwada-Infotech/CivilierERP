...
function makeRecentCols(
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ getValue }: any) => (
        <span className="text-xs text-muted-foreground max-w-[110px] truncate block">{(getValue() as string) || "—"}</span>
      ),
    },
    {
      accessorKey: dateKey,
    ...
    },
    {
      ...
      cell: ({ getValue }: any) => {
        ...
        return <span className="text-xs font-medium">{v != null ? fmt(v) : "—"}</span>;
      },
    },
    {
      accessorKey: statusKey,
      header: "Status",
      cell: ({ getValue }: any) => <StatusBadge status={(getValue() as string) || defaultStatus} />,
    },
  ];
}

const GRN_DASH_COLS = makeRecentCols("GRNNo",           "GRNID",           "SupplierName", "GRNDate",      "TotalAmount", "Status");
const PO_DASH_COLS  = makeRecentCols("PurchaseOrderNo", "PurchaseOrderID", "SupplierName", "PODate",       "TotalAmount", "Status");
const WO_DASH_COLS  = makeRecentCols("DocumentNumber",  "Id",              "ProjectName",  "DocumentDate", "TotalAmount", "Status");
const EXP_DASH_COLS = makeRecentCols("EDocNo",          "Eid",             "EProjectName", "EDocDate",     "EAmount",     "EStatus", "Pending");
