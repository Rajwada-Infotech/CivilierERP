...
export default function L1Chart() {
  ...
  // Effective winning supplier for display
  ...
  return (
    ...
    >
      ...
      {/* ── Toolbar ─────────────────────────────────────────────────────── */}
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center bg-card border border-border rounded-xl px-4 py-3">
        {/* Quotation picker */}
        <div className="flex-1 flex items-center gap-2 min-w-0">
          ...
          <div className="relative flex-1 min-w-0">
            ...
            >
              <option value="">
                {loadingQuotations ? "Loading…" : comparableQuotations.length === 0 ? "No sent quotations yet" : "Select…"}
              </option>
        ...
        {/* Add supplier — only shown when a quotation is selected */}
        {quotationId && (
          <>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Button
                type="button"
                size="sm"
                className="gap-1 bg-gradient-to-r from-emerald-500 to-teal-400 text-white hover:opacity-90 border-0 h-[42px] px-3"
          ...
          {!hasChart ? (
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {chart!.suppliers.map((s) => (
                  ...
                  >
                    ...
                    {s.SupplierName}
                    {s.SupplierLHeadId === cheapestSupplierId && <Trophy size={10} className="text-amber-400" />}
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="bg-muted/50 border-b border-border">
                        <th className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-muted-foreground sticky left-0 bg-muted/50 min-w-[180px]">
                          Item
                        </th>
                        <th className="px-3 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground text-center whitespace-nowrap">
                          UOM · Qty
                        </th>
                        {chart!.suppliers.map((s) => (
                          <th
                            key={s.SupplierLHeadId}
                            className={`px-4 py-3 text-left min-w-[150px] ${
                          ...
                          >
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-xs font-semibold ${s.SupplierLHeadId === cheapestSupplierId ? "text-emerald-600 dark:text-emerald-400" : ""}`}>
                                {s.SupplierName}
                              </span>
                              {s.SupplierLHeadId === cheapestSupplierId && (
                            ...
                            {supplierTotals[s.SupplierLHeadId] > 0 && (
                              <div className={`text-[10px] mt-0.5 font-medium ${s.SupplierLHeadId === cheapestSupplierId ? "text-emerald-500" : "text-muted-foreground"}`}>
                    ...
                    <tbody>
                      {chart!.items.map((item, idx) => (
                        ...
                        >
                          <td className="px-5 py-3 font-medium sticky left-0 bg-card text-sm">
                            {item.ItemName}
                          </td>
                          <td className="px-3 py-3 text-center text-xs text-muted-foreground whitespace-nowrap">
                            <div>{item.UOMName || item.UOMCode}</div>
                          ...
                          {chart!.suppliers.map((s) => {
                            ...
                            return (
                              ...
                              >
                                {p ? (
                                  <div>
                                    <div className={`font-semibold flex items-center gap-1 ${isL1 ? "text-emerald-600 dark:text-emerald-400" : ""}`}>
                                      {isL1 && <Trophy size={10} className="text-amber-400 shrink-0" />}
                                      {fmtAmt(p.Rate)}
                                    </div>
                                    {p.SupplyDate && (
                    ...
                    <tfoot>
                      <tr className="bg-muted/60 border-t-2 border-border">
                        <td className="px-5 py-3 text-xs font-bold uppercase tracking-widest text-muted-foreground sticky left-0 bg-muted/60">
                          Grand Total
                        </td>
                        <td />
                        {chart!.suppliers.map((s) => (
                          ...
                          >
                            <div className={`font-bold text-sm ${s.SupplierLHeadId === cheapestSupplierId ? "text-emerald-600 dark:text-emerald-400" : ""}`}>
              ...
              {submittedSuppliers.length > 0 && (
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-card border border-border rounded-xl px-4 py-3">
                  <div className="flex items-center gap-2 shrink-0">
                  ...
                  {effectiveWinner && (
                    <div className="text-xs text-muted-foreground hidden sm:block">
                      ...
                      <span className="font-semibold text-foreground">
                        {fmtAmt(supplierTotals[effectiveWinner.SupplierLHeadId] ?? 0)}
                      </span>
                    </div>
                  )}
                  <Button
                    type="button"
                    onClick={createPO}
                    className="gap-1.5 bg-gradient-to-r from-emerald-500 to-teal-400 text-white hover:opacity-90 border-0 shrink-0 sm:ml-auto"
                  >
