...
export default function PortalLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate(data.mustChangePassword ? "/crm-client-portal/change-password" : "/crm-client-portal/dashboard", { replace: true });
      }, 1800);
    } catch (err: any) {
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] p-12 relative overflow-hidden"
        ...
        <motion.div className="relative z-10" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.15 }}>
          ...
          <p className="text-violet-200/70 text-sm leading-relaxed max-w-xs">
            Track your booking, review and approve your agreement, and follow every milestone — from token to handover.
          </p>

          <div className="mt-8 space-y-3">
            {[
              { icon: "🏠", text: "Track your booking & unit details" },
              { icon: "📄", text: "Review and approve your agreement" },
              { icon: "💳", text: "Follow payment milestones in real time" },
              { icon: "🎫", text: "Raise and track support tickets" },
            ].map((f, i) => (
