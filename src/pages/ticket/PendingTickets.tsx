...
type StatusFilter = "All Tickets" | "Pending" | "InProgress" | "Resolved" | "Closed";
...
const STATUS_TABS: StatusFilter[] = ["Pending", "InProgress"];
...
const PendingTickets: React.FC = () => {
  ...
  const tickets = useMemo(() => {
    let list = [...allTickets];
    if (statusFilter !== "All Tickets") list = list.filter((t) => t.status === statusFilter);
    if (priorityFilter !== "All") list = list.filter((t) => t.priority === priorityFilter);
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(
        (t) => t.subject.toLowerCase().includes(q) || t.customer_name?.toLowerCase().includes(q) || t.issue_details?.toLowerCase().includes(q),
      );
    }
    return list;
  ...
  const isFiltered = statusFilter !== "Pending" || priorityFilter !== "All" || !!search.trim();

  // ── Detail view ──────────────────────────────────────────────────────────────
  if (selectedTicketId !== null) {
    return (
      <>
        <Breadcrumbs items={["Tickets", "Pending Tickets", `#${selectedTicketId}`]} />
      ...
      <div className="max-w-3xl mx-auto pb-10 space-y-5">
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-border bg-card py-16 flex flex-col items-center gap-3 text-muted-foreground">
            ...
            <p className="text-sm">{isFiltered ? "No tickets match your filters" : "No pending tickets"}</p>
            {isFiltered && (
              <button onClick={() => { setSearch(""); setPriorityFilter("All"); setStatusFilter("Pending"); }} className="text-xs text-primary hover:underline">
                Clear filters
              </button>
            )}
          </div>
