...
const PAGE_SIZE = 20;
interface RegistryListFilters {
  search: string;
  companyId: string;
  projectId: string;
  blockId: string;
}
async function fetchRegistryList(filters: RegistryListFilters, page: number): Promise<{ rows: any[]; total: number }> {
...
const CrmRegistry: React.FC = () => {
  ...
  const listFilters: RegistryListFilters = useMemo(
  ...
  const { data: listResult, isLoading, dataUpdatedAt, isFetching, refetch } = useQuery({
    queryKey: ["crm-registry", listFilters, page],
    queryFn: () => fetchRegistryList(listFilters, page),
    staleTime: 30_000,
  });
          ...
          <div className="w-80 shrink-0 flex flex-col gap-2">
            <div className="relative">
              ...
              <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)}
                ...
                placeholder="Search registrations... (Enter to search)"
