...
const MODULE_OPTIONS = [
  "General", "Finance", "Material", "Engineering",
  "Follow-Up", "Ticket", "Masters", "Reports",
];
...
async function createDef(body: Omit<FormState, "sortOrder"> & { sortOrder: number; isActive: boolean }) {
...
async function updateDef(id: number, body: Omit<FormState, "sortOrder"> & { sortOrder: number; isActive: boolean }) {
...
async function deleteDef(id: number) {
  const res = await fetchWithAuth(`/api/page-definitions/${id}`, { method: "DELETE" });
...
export default function PageDefinitionsAdmin() {
  ...
  const { data: rows = [], isLoading } = useQuery<PageDef[]>({
  ...
  // ── Mutations ──────────────────────────────────────────────────────────────
  const invalidate = () => qc.invalidateQueries({ queryKey: ["page-definitions-all"] });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      if (editingId !== null) {
        ...
        await updateDef(editingId, { ...payload, isActive: existing?.isActive ?? true });
      } else {
        await createDef(payload);
      }
    },
    onSuccess: () => {
      toast.success(editingId !== null ? "Page definition updated" : "Page definition created");
      closeDrawer();
      invalidate();
    },
  ...
  const toggleActiveMut = useMutation({
    mutationFn: async ({ row, newActive }: { row: PageDef; newActive: boolean }) => {
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => { toast.success("Page definition deactivated"); setDeleteTarget(null); invalidate(); },
    onError:   (err: Error) => toast.error(err.message),
  });
  ...
  function closeDrawer() { setDrawerOpen(false); setEditingId(null); setForm(emptyForm); }
  ...
  const modules = useMemo(() => ["All", ...Array.from(new Set<string>(rows.map(r => r.module)))], [rows]);
    ...
    <>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-xl font-heading font-bold text-foreground leading-none">Page Definitions</h1>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="bg-card border border-border rounded-xl px-4 py-3">
            <p className="text-[10px] text-muted-foreground font-heading uppercase tracking-wide">{s.label}</p>
            <p className={`text-xl font-heading font-bold mt-0.5 ${s.color}`}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-sm text-muted-foreground">No page definitions found</div>
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="px-4 py-2.5 text-left font-heading font-semibold text-muted-foreground uppercase tracking-wide text-[10px]">Key</th>
                <th className="px-4 py-2.5 text-left font-heading font-semibold text-muted-foreground uppercase tracking-wide text-[10px]">Label</th>
                <th className="px-4 py-2.5 text-left font-heading font-semibold text-muted-foreground uppercase tracking-wide text-[10px]">Module / Group</th>
                <th className="px-4 py-2.5 text-left font-heading font-semibold text-muted-foreground uppercase tracking-wide text-[10px]">Actions</th>
                <th className="px-4 py-2.5 text-center font-heading font-semibold text-muted-foreground uppercase tracking-wide text-[10px]">Order</th>
                <th className="px-4 py-2.5 text-center font-heading font-semibold text-muted-foreground uppercase tracking-wide text-[10px]">Status</th>
                <th className="px-4 py-2.5 text-center font-heading font-semibold text-muted-foreground uppercase tracking-wide text-[10px]">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(row => {
                const colorClass = MODULE_COLORS[row.module] ?? "bg-muted text-muted-foreground border-muted";
                return (
                  <tr key={row.id} className={`border-b border-border/40 hover:bg-muted/20 transition-colors ${!row.isActive ? "opacity-50" : ""}`}>
                    <td className="px-4 py-2.5 font-mono text-[11px] text-primary font-semibold">{row.key}</td>
                    <td className="px-4 py-2.5 text-sm font-body text-foreground">{row.label}</td>
                    <td className="px-4 py-2.5">
                      <span className={`inline-flex text-[10px] font-semibold px-2 py-0.5 rounded-full border ${colorClass}`}>
                        {row.module}
                      </span>
                      <span className="ml-1.5 text-[10px] text-muted-foreground">{row.group}</span>
                    </td>
                    <td className="px-4 py-2.5">
                      <div className="flex flex-wrap gap-1">
                        {row.actions.map(a => (
                          <span key={a} className="px-1.5 py-0.5 rounded text-[9px] bg-primary/10 text-primary font-medium capitalize">
                            {a}
                          </span>
                        ))}
                      </div>
                    </td>
                    <td className="px-4 py-2.5 text-center text-muted-foreground">{row.sortOrder}</td>
                    <td className="px-4 py-2.5 text-center">
                      <button
                        onClick={() => toggleActiveMut.mutate({ row, newActive: !row.isActive })}
          ...
          <div className="relative w-full max-w-sm bg-card border-l border-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-heading font-semibold text-sm text-foreground">
                  {editingId !== null ? "Edit Page Definition" : "New Page Definition"}
                </h3>
              </div>
              <button onClick={closeDrawer} className="p-1.5 rounded-lg text-muted-foreground hover:bg-muted">
                <X size={14} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1">Must match the route key used in permission checks. Cannot be changed after creation.</p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <svg className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs rounded-lg border border-border bg-background focus:outline-none focus:ring-1 focus:ring-primary/30"
                />
                <p className="text-[10px] text-muted-foreground mt-1">Pages with the same group name are shown under the same heading in Menu Rights.</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  min={1}
                />
                <p className="text-[10px] text-muted-foreground mt-1">Lower numbers appear first within their group.</p>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  ...
                  {saveMut.isPending ? "Saving…" : editingId !== null ? "Update" : "Create"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-heading font-semibold text-sm text-foreground">Deactivate Page Definition?</h3>
                <p className="text-xs text-muted-foreground mt-1 font-body">
                  <span className="font-mono font-semibold text-foreground">{deleteTarget.key}</span> will be hidden from Menu Rights.
                  Existing user permissions for this page are preserved but won't take effect.
                </p>
              </div>
            </div>
