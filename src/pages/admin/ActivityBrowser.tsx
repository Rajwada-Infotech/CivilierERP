...
const ActivityBrowser: React.FC = () => {
  ...
  // Update local dateRange when dateFilters change
  useEffect(() => {
    setDateRange({
      from: dateFilters.dateFrom ? new Date(dateFilters.dateFrom + 'T00:00:00') : undefined,
      to: dateFilters.dateTo ? new Date(dateFilters.dateTo + 'T00:00:00') : undefined,
    });
  }, [dateFilters]);
  ...
  const handleDateRangeChange = (range: { from: Date | undefined; to: Date | undefined }) => {
    ...
    if (range.from && range.to) {
      // Use date-fns format to prevent timezone shifts
      setDateFilters({
  ...
  const chartData = useMemo(() => {
    ...
    const dataMap: Record<string, { date: string; actions: number; logins: number; fullDate: string }> = {};
    ...
    if (dateRange.from) {
      ...
      const timestamps = rawSessions.map(s => new Date(s.timestamp).getTime());
    ...
    // Pre-fill range buckets
    const curr = new Date(start);
    let safety = 0;
    while (curr <= end && safety < 90) { // Limit to 90 days for chart readability
  ...
  const analytics = useMemo(() => {
    ...
    return {
      topUsers: Object.entries(userCounts).sort((a, b) => b[1] - a[1]).slice(0, 3),
      topResources: Object.entries(resourceCounts).sort((a, b) => b[1] - a[1]).slice(0, 3)
    };
  }, [rawSessions]);
  ...
  const filteredSessions = useMemo(() => {
    ...
    return groupedSessions.filter((session) => {
      ...
      const actionTypeMatch = !quickFilter || session.actions.some(a => a.actionType === quickFilter);
      const searchMatch =
        !q ||
  ...
  const currentPeriodLabel = dateFilters.period 
    ...
    : dateFilters.dateFrom || dateFilters.dateTo ? 'Custom Range' : 'Last Month';
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Login, logout, device fingerprint and everything changed during each session.
          </p>
        </div>

        <div className="rounded-md border border-border bg-muted px-3 py-1.5 text-xs text-muted-foreground">
          {groupedSessions.length} sessions · {rawSessions.filter(e => e.event === 'action').length} actions | {currentPeriodLabel}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <Select
            ...
            onValueChange={(v) => handlePresetClick(v as ActivityLogFilters["period"])}
          >
          ...
          <div className="flex items-center gap-1 border-l border-border pl-3 ml-1">
            {(['create', 'update', 'delete'] as ActivityActionType[]).map((act) => (
              <Button key={act} variant={quickFilter === act ? "default" : "outline"} size="sm" className="h-7 text-[10px] uppercase tracking-wider" onClick={() => handleQuickFilter(act)}>
                {act}
              </Button>
            ))}
          </div>
            ...
            <PopoverTrigger asChild>
              ...
              >
                ...
                {dateRange.from ? (
                  dateRange.to ? (
                    <>
                      {dateRange.from.toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })} -{" "}
                      {dateRange.to.toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </>
                  ) : (
    ...
    {!isLoading && (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="border-border/40 bg-card/20 backdrop-blur-sm">
          ...
          <CardContent className="px-4 pb-4">
            <div className="space-y-2">
              {analytics.topUsers.length > 0 ? analytics.topUsers.map(([user, count]) => (
                <div key={user} className="flex items-center justify-between p-2 rounded-lg bg-muted/40 text-sm">
                  <span className="font-medium">{user}</span>
                  <Badge variant="secondary" className="text-[10px]">{count} actions</Badge>
                </div>
              )) : <div className="text-xs text-muted-foreground py-2 italic text-center">No action data</div>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/40 bg-card/20 backdrop-blur-sm">
          ...
          <CardContent className="px-4 pb-4">
            <div className="space-y-2">
              {analytics.topResources.length > 0 ? analytics.topResources.map(([res, count]) => (
                <div key={res} className="flex items-center justify-between p-2 rounded-lg bg-muted/40 text-sm">
                  <span className="font-medium capitalize">{res}</span>
                  <Badge variant="outline" className="text-[10px] border-amber-500/20 text-amber-600">{count} hits</Badge>
                </div>
              )) : <div className="text-xs text-muted-foreground py-2 italic text-center">No resource data</div>}
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Legend 
                    ...
                    align="right" 
                    iconType="circle"
                    wrapperStyle={{ paddingBottom: 20, fontSize: 11, fontWeight: 500 }}
                  />
                  <Bar dataKey="actions" name="Actions" fill="hsl(var(--primary))" radius={[2, 2, 0, 0]} barSize={24} />
                  <Bar dataKey="logins" name="Logins" fill="#10b981" radius={[2, 2, 0, 0]} barSize={24} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
      ...
      {isLoading ? (
        ...
        filteredSessions.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-border bg-muted/30 py-24 text-muted-foreground">
            ...
            <p className="text-lg font-heading font-semibold">No session data found</p>
            <p className="text-sm">Try adjusting the date range or other filters above.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredSessions.map((session) => {
              ...
              const alerts = [];
              const uniqueIps = new Set(session.actions.map(a => a.ipAddress).filter(Boolean));
              uniqueIps.add(session.loginEvent.ipAddress);
              if (session.logoutEvent) uniqueIps.add(session.logoutEvent.ipAddress);
              
              if (uniqueIps.size > 1) {
                alerts.push({ icon: ShieldAlert, label: "Multiple IPs", color: "text-amber-500" });
              }
                  ...
                  <div className="grid gap-4 border-b border-border bg-muted/30 p-5 lg:grid-cols-[1.3fr_1fr_1fr_1fr]">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`rounded-full border px-2 py-0.5 text-xs font-heading ${ROLE_COLORS[session.userRole] || ROLE_COLORS.user}`}
                        >
                        ...
                        <span className={cn(
                          ...
                          {session.logoutTime ? "Completed" : "Active Now"}
                        </span>
                      </div>
                      <p className="text-sm text-muted-foreground">{session.userEmail}</p>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {alerts.map((alert, idx) => (
                          <div key={idx} className={cn("flex items-center gap-1 text-[10px] font-bold uppercase", alert.color)}>
                            <alert.icon size={12} />
                            {alert.label}
                          </div>
                        ))}
                      ...
                      <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground">
                        <Fingerprint size={13} />
                        <span className="break-all">{session.deviceFingerprint}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-sm text-muted-foreground">{loginMeta.date}</div>
                      <div className="text-sm font-mono">{loginMeta.time}</div>
                      <div className="text-xs text-muted-foreground">
                        IP: {session.loginEvent.ipAddress}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-sm text-muted-foreground">{logoutMeta.date}</div>
                      <div className="text-sm font-mono">{logoutMeta.time}</div>
                      <div className="text-xs text-muted-foreground">
                        IP: {session.logoutEvent?.ipAddress || "Still active"}
                      </div>
                    </div>
                  ...
                  <div className="p-5">
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <h3 className="font-heading text-sm font-semibold text-foreground">
                        What user did in this login session
                      </h3>
                    </div>

                    {session.actions.length === 0 ? (
                      <div className="rounded-xl border border-dashed border-border bg-muted/20 p-6 text-center text-sm text-muted-foreground">
                        No tracked actions inside this session yet.
                      </div>
                    ) : (
                          ...
                          <tbody>
                            {session.actions.map((action) => (
                              <tr key={action.id} className="border-b border-border/60">
                                ...
                                <td className="px-3 py-3 capitalize">{action.resource || "—"}</td>
            ...
            <tbody>
              {filteredActions.map((action) => (
                <tr key={action.id} className="border-t border-border/70">
                  ...
                  <td className="px-4 py-3 capitalize">{action.resource || "—"}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <Calendar size={12} />
      ...
      {!isLoading && (
        <div className="grid grid-cols-2 gap-4 border-t border-border pt-6 text-xs md:grid-cols-4">
          <div className="text-center">
            <div className="text-lg font-heading text-foreground">{groupedSessions.length}</div>
            <div className="text-muted-foreground">Total Sessions</div>
          </div>
          <div className="text-center">
            <div className="text-lg font-heading text-foreground">{rawSessions.filter(e => e.event === 'action').length}</div>
