...
export default function Login() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center px-4 py-8 overflow-auto"
      style={{ background: "linear-gradient(135deg, #f3e8ff 0%, #e9d5ff 25%, #ffffff 60%, #f8f4ff 100%)" }}>
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full blur-[120px]" style={{ background: "rgba(168,85,247,0.25)" }} />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full blur-[120px]" style={{ background: "rgba(139,92,246,0.2)" }} />
      </div>

      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.2, 0, 0, 1] }}
        className="relative z-10 w-full max-w-md p-6 sm:p-8 shadow-2xl rounded-2xl"
        style={{ background: "linear-gradient(90deg,#d1d5db 0%,#e5e7eb 40%,#f9fafb 75%,#ffffff 100%)", border: "1px solid rgba(255,255,255,0.5)", boxShadow: "0 25px 60px rgba(139,92,246,0.12)" }}>

        <div className="text-center mb-6 sm:mb-8">
          <div className="flex flex-col items-center gap-3">
            <img src="/CivilierERP.png" alt="CivilierERP Logo"
          ...
          <p className="mt-2 text-xs sm:text-sm text-slate-600">Enterprise Resource Planning — Built for Civil Contractors</p>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div>
            ...
            <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(""); }} required
              ...
              className="w-full rounded-lg px-4 py-2.5 border border-purple-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-300/50 transition-all bg-white/70 text-slate-800 placeholder:text-slate-400 text-sm" />
          </div>
          <div className="relative">
            ...
            <input type={showPass ? "text" : "password"} value={password} onChange={e => { setPassword(e.target.value); setError(""); }} required
              placeholder="••••••••"
              className="w-full rounded-lg px-4 py-2.5 pr-11 border border-purple-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-300/50 transition-all bg-white/70 text-slate-800 placeholder:text-slate-400 text-sm" />
          ...
          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                className="px-4 py-2.5 rounded-lg bg-red-50 border border-red-200 text-sm text-red-600">{error}</motion.div>
            )}
          </AnimatePresence>

          <button type="submit"
            className="w-full py-3 rounded-lg font-semibold text-sm sm:text-base text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-200"
        ...
        <div className="mt-5">
          <button type="button" onClick={() => setShowHints(!showHints)}
            ...
            <ShieldCheck size={14} /> {showHints ? "Hide demo credentials" : "Show demo credentials"}
          </button>
          <AnimatePresence>
            {showHints && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                ...
                {ROLE_HINTS.map(h => (
                  <button key={h.email} type="button" onClick={() => { setEmail(h.email); setPassword(h.password); setShowHints(false); setError(""); }}
