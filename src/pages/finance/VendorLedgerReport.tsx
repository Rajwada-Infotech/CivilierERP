/**
 * VendorLedgerReport.tsx — Finance → Vendor Ledger Report
 *
...
export default function VendorLedgerReport() {
  ...
  const loading = summaryQuery.isFetching || transactionsQuery.isFetching;
  ...
  }, [summaryQuery.error, transactionsQuery.error]);

  const meta = useMemo(() => typeMeta(selectedHead?.Type ?? null), [selectedHead]);
  const MetaIcon = meta.icon;

  return (
    <>
      <Breadcrumbs items={["Dashboard", "Finance", "Vendor Ledger Report"]} />
      <FinanceShell
      ...
      >
        ...
        {!selectedHead ? (
          <div className="glass rounded-xl ring-1 ring-border/60 py-16 flex flex-col items-center justify-center gap-2 text-muted-foreground">
            ...
            <p className="text-sm">Search and pick a party or GL account above to see its ledger.</p>
          </div>
        ) : (
          <>
            ...
            <div className="glass rounded-xl ring-1 ring-border/60 overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-5 py-4 border-b border-border/60">
                ...
                <ExportMenu
                  ...
                  title={`Vendor Ledger — ${selectedHead.Name ?? ""}`}
                  filename="vendor-ledger-report"
                  disabled={transactionsQuery.isFetching || !transactions.length || !rights.canExport}
                />
              </div>
                    ...
                    <table className="w-full text-xs min-w-[760px]">
                      <thead>
                        <tr className="border-b border-border/60 text-muted-foreground uppercase tracking-wide text-[10px] font-heading">
                          ...
                          <th className="text-right px-4 sm:px-5 py-2.5">Balance</th>
                        </tr>
                      </thead>
                          ...
                          return (
                            <tr key={t.EntryId} className="hover:bg-muted/20 transition-colors">
                              ...
                              <td className="px-4 sm:px-5 py-2.5 text-right tabular-nums font-semibold text-foreground">
                                {formatINR(Number(t.RunningBalance))}
                              </td>
                            </tr>
                          );
                        })}
