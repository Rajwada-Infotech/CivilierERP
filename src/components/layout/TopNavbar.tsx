...
import {
  Settings, BarChart3, LogOut, User, Palette,
  LayoutGrid, Puzzle, ShieldCheck, Crown, Shield,
  CheckCircle2, Menu, X, Receipt, Truck, Users, HardHat, Landmark,
...
const masterItems = [
  { icon: Receipt,  label: "Expenses",    path: "/masters/expenses",    color: "text-orange-400" },
...
export const TopNavbar = () => {
  ...
  const closeAll = () => {
    setSetupOpen(false); setModuleOpen(false);
    setUserOpen(false);  setThemeOpen(false);
  };

  const handleSetupClick = useCallback(() => {
    if (!isModuleActive) return;
    setSetupOpen(p => !p);
    setModuleOpen(false); setUserOpen(false); setThemeOpen(false);
  }, [isModuleActive]);
    ...
    <header className="fixed top-0 left-0 right-0 h-14 z-50 flex items-center justify-between px-4 border-b border-border bg-card/80 backdrop-blur-lg">
      ...
      <div className="hidden md:flex items-center gap-1">
        ...
        >
          ...
          <div className="relative shrink-0">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-heading transition-all duration-200 whitespace-nowrap ${
                isModuleActive ? "hover:bg-muted text-foreground" : "text-muted-foreground cursor-not-allowed opacity-40"
              }`}
              title={isModuleActive ? "Setup" : "Select a module first"}
            >
              <Settings size={16} /> Setup
            </button>
            <Dropdown open={setupOpen} onClose={() => setSetupOpen(false)} className="right-0 w-72 p-3">
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-heading mb-3">Masters</p>
              <div className="grid grid-cols-3 gap-2">
                {masterItems.map(({ icon: Icon, label, path, color }) => (
                  <button key={label} onClick={() => { navigate(path); setSetupOpen(false); }}
                    className={`group flex flex-col items-center gap-2 p-3 rounded-xl border transition-all ${
                      location.pathname === path ? "border-primary/60 bg-primary/10" : "border-border/50 hover:border-border hover:bg-muted/60"
                    }`}>
                    ...
                    <span className="text-[11px] font-heading text-muted-foreground group-hover:text-foreground transition-colors text-center">{label}</span>
                  </button>
                ))}
              </div>
            </Dropdown>
          ...
          {/* Reports */}
          <button onClick={() => navigate("/reports")} className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-heading transition-all duration-200 whitespace-nowrap ${location.pathname === "/reports" ? "bg-primary/10 text-primary" : "hover:bg-muted text-foreground"}`}>
            <BarChart3 size={16} /> Reports
          </button>

          {/* Widgets */}
          <button onClick={() => navigate("/widgets")} className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-heading transition-all duration-200 whitespace-nowrap ${location.pathname === "/widgets" ? "bg-primary/10 text-primary" : "hover:bg-muted text-foreground"}`}>
            <Puzzle size={16} /> Widgets
          </button>

          {/* Tasks */}
          <button onClick={() => navigate("/tasks")} className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-heading transition-all duration-200 whitespace-nowrap ${location.pathname.startsWith("/tasks") ? "bg-primary/10 text-primary" : "hover:bg-muted text-foreground"}`}>
        ...
        {/* Module — always outside collapsible so dropdown isn't clipped */}
        <div className="relative shrink-0">
          <button onClick={() => { setModuleOpen(p => !p); setSetupOpen(false); setUserOpen(false); setThemeOpen(false); }}
          ...
          <Dropdown open={moduleOpen} onClose={() => setModuleOpen(false)} className="right-0 w-80 p-3">
            ...
            <div className={`grid gap-3 ${isAdmin ? "grid-cols-2" : "grid-cols-1"}`}>
              <button onClick={() => { setActiveModule("finance"); setModuleOpen(false); navigate("/"); }}
                ...
                <span className="text-xs font-heading text-muted-foreground group-hover:text-foreground transition-colors">Finance</span>
                {activeModule === "finance" && <span className="text-[10px] text-primary font-heading">Active</span>}
              </button>
              {isAdmin && (
                <button onClick={() => { setModuleOpen(false); navigate("/admin"); }}
                  ...
                  <span className="text-xs font-heading text-muted-foreground group-hover:text-foreground transition-colors">Admin</span>
        ...
        {/* Theme */}
        <div className="relative shrink-0">
          <button onClick={() => { setThemeOpen(p => !p); setSetupOpen(false); setModuleOpen(false); setUserOpen(false); }}
            ...
            <span className="w-3 h-3 rounded-full shrink-0" style={{ background: THEME_DOTS[theme].bg }} />
          ...
          <Dropdown open={themeOpen} onClose={() => setThemeOpen(false)} className="right-0 w-48 p-1.5">
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-heading px-2 py-1.5 mb-0.5">Appearance</p>
        ...
        {/* User */}
        <div className="relative shrink-0">
          <button onClick={() => { setUserOpen(p => !p); setModuleOpen(false); setSetupOpen(false); setThemeOpen(false); }}
          ...
          <Dropdown open={userOpen} onClose={() => setUserOpen(false)} className="right-0 w-56 p-1">
            <div className="px-3 py-2 border-b border-border mb-1">
              ...
              <div className="mt-1.5">
                ...
                {currentUser?.role === "user" && <span className="text-[10px] px-2 py-0.5 rounded-full font-heading bg-muted text-muted-foreground">User · {currentUser.pagePermissions.length} pages</span>}
              </div>
            </div>
            ...
            <button onClick={() => logout()} className="w-full flex items-center gap-2 px-3 py-2 text-sm rounded-md hover:bg-muted transition-colors text-destructive">
