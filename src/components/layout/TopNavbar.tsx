...
import {
  Settings, BarChart3, LogOut, User,
  LayoutGrid, Puzzle, ShieldCheck, Crown, Shield,
  CheckCircle2, Menu, X, Receipt, Truck, Users, HardHat, Landmark, Package,
} from "lucide-react";
...
export const TopNavbar = () => {
  ...
  const navLinks = [
    ...
    { label: "Tasks", onClick: () => navigate("/tasks"), path: "/tasks", badge: overdueCount },
  ];
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-14 z-50 flex items-center justify-between px-4 border-b border-border bg-card/80 backdrop-blur-lg">
      ...
      <div className="hidden md:flex items-center gap-1">
            ...
            <div className="grid grid-cols-3 gap-2">
              {masterItems.map(({ icon: Icon, label, path, color }) => (
                ...
                >
                  ...
                  <span className="text-[11px] font-heading text-muted-foreground group-hover:text-foreground transition-colors leading-tight text-center">{label}</span>
                </button>
              ))}
            </div>
          </Dropdown>
        ...
        {/* Module Selector */}
        <div className="relative">
          <button onClick={() => { setModuleOpen(p => !p); setSetupOpen(false); setUserOpen(false); }}
        ...
        <div className="relative">
          <button
            onClick={() => { setThemeOpen(p => !p); setSetupOpen(false); setModuleOpen(false); setUserOpen(false); }}
            className="flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-heading hover:bg-muted transition-colors text-foreground"
          >
          ...
          <Dropdown open={themeOpen} onClose={() => setThemeOpen(false)} className="right-0 w-48 p-1.5">
            ...
            {(Object.entries(THEME_DOTS) as [Theme, { bg: string; label: string }][]).map(([t, { bg, label }]) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-heading transition-colors ${theme === t ? "bg-primary/10 text-primary" : "text-foreground hover:bg-muted"}`}
              >
        ...
        {/* User */}
        <div className="relative">
          <button onClick={() => { setUserOpen(p => !p); setModuleOpen(false); setSetupOpen(false); }}
      ...
      {mobileMenuOpen && (
        <div className="absolute top-14 left-0 right-0 bg-card border-b border-border shadow-xl md:hidden z-40 p-3 space-y-1">
          ...
          <button onClick={() => { navigate("/transactions"); setMobileMenuOpen(false); }}
          ...
          <button onClick={() => { navigate("/widgets"); setMobileMenuOpen(false); }}
          ...
          <button onClick={() => { navigate("/tasks"); setMobileMenuOpen(false); }}
            ...
            <span>Tasks</span>
            {overdueCount > 0 && <span className="px-1.5 py-0.5 rounded-full bg-red-500 text-white text-[10px] font-bold">{overdueCount}</span>}
          </button>
          {isAdmin && (
          ...
          <div className="border-t border-border pt-2 mt-2 space-y-1">
            <p className="px-3 text-[10px] uppercase tracking-widest text-muted-foreground font-heading py-1">Masters</p>
            {masterItems.map(({ icon: Icon, label, path, color }) => (
          ...
          <div className="border-t border-border pt-2 mt-2">
            <p className="px-3 text-[10px] uppercase tracking-widest text-muted-foreground font-heading py-1.5">Appearance</p>
            <div className="space-y-0.5">
              {(Object.entries(THEME_DOTS) as [Theme, { bg: string; label: string }][]).map(([t, { bg, label }]) => (
                <button
                  key={t}
                  onClick={() => { setTheme(t); setMobileMenuOpen(false); }}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-heading transition-colors ${theme === t ? "bg-primary/10 text-primary" : "text-foreground hover:bg-muted"}`}
                >
                  <span className="w-3.5 h-3.5 rounded-full shrink-0 border border-border/50" style={{ background: bg }} />
                  {label}
