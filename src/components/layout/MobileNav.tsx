        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 rounded-t-2xl bg-card border-t border-border animate-slide-up max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-3 space-y-0.5">
              {isAdminPage ? (
                <p className="text-xs text-muted-foreground text-center py-6 font-heading">Admin Module — no sub-navigation</p>
              ) : (
                <>
                  {/* Dashboard */}
                  <button onClick={() => go("/")}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-heading transition-colors ${location.pathname === "/" ? "bg-primary/15 text-primary font-medium" : "text-foreground hover:bg-muted"}`}>
                    <BarChart3 size={18} /> Dashboard
                  </button>

                  {/* Query group */}
                  <div>
                    <button onClick={() => setQueryOpen(p => !p)}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-heading transition-colors ${location.pathname === "/transactions" ? "text-primary" : "text-foreground hover:bg-muted"}`}>
                      <Scale size={18} />
                    ...
                    <div className={`overflow-hidden transition-all duration-200 ${queryOpen ? "max-h-20 opacity-100" : "max-h-0 opacity-0"}`}>
                      <div className="ml-4 pl-3 border-l border-border py-0.5">
                        <button onClick={() => go("/transactions")}
                          className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-heading transition-colors ${location.pathname === "/transactions" ? "bg-primary/15 text-primary font-medium" : "text-foreground hover:bg-muted"}`}>
                  ...
                  <button onClick={() => go("/tasks")}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-heading transition-colors ${location.pathname.startsWith("/tasks") ? "bg-primary/15 text-primary font-medium" : "text-foreground hover:bg-muted"}`}>
                  ...
                  {isAdmin && (
                    <button onClick={() => go("/admin")}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-heading transition-colors ${location.pathname.startsWith("/admin") ? "bg-primary/15 text-primary font-medium" : "text-foreground hover:bg-muted"}`}>
                  ...
                  <div className="border-t border-border pt-2 mt-1">
                    <p className="px-3 text-[10px] uppercase tracking-widest text-muted-foreground font-heading py-1.5">Masters</p>
              ...
              <div className="border-t border-border pt-2 mt-1">
                <button onClick={() => setThemeOpen(p => !p)}
                  ...
                  <span className="w-3.5 h-3.5 rounded-full border border-border/50 shrink-0" style={{ background: THEME_DOTS[theme].bg }} />
                  {themeOpen ? <ChevronUp size={14} className="text-muted-foreground" /> : <ChevronDown size={14} className="text-muted-foreground" />}
                </button>
                <div className={`overflow-hidden transition-all duration-200 ${themeOpen ? "max-h-64 opacity-100" : "max-h-0 opacity-0"}`}>
                  <div className="ml-4 pl-3 border-l border-border py-0.5 space-y-0.5">
                    {(Object.entries(THEME_DOTS) as [Theme, { bg: string; label: string }][]).map(([t, { bg, label }]) => (
                      <button key={t} onClick={() => { setTheme(t); setThemeOpen(false); }}
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-heading transition-colors ${theme === t ? "bg-primary/10 text-primary" : "text-foreground hover:bg-muted"}`}>
