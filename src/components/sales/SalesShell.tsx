...
/**
 * Shared glass-themed wrapper for all Sales module pages.
 * Provides ambient violet glow, frosted header band, and consistent layout —
 * mirrors FinanceShell/MaterialShell, recolored to match the Sales module's
 * ModuleStrip accent (#a855f7).
 */
export const SalesShell: React.FC<SalesShellProps> = ({
  title,
  subtitle,
  icon: PageIcon,
  action,
  ...
  const glassCard = isDark
    ...
    : {
        ...
        boxShadow:
          "0 8px 32px rgba(168,85,247,0.08), inset 0 1px 0 rgba(255,255,255,0.9)",
      };
    ...
    <div className="relative min-h-full p-4 space-y-5">
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to bottom, transparent 10%, #a855f7 30%, #a855f7 70%, transparent 90%)",
          }}
        />
        ...
        <div className="relative z-10 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
          <div className="flex items-center gap-3">
            ...
            >
              {PageIcon ? (
                <PageIcon size={16} style={{ color: "#c084fc" }} />
              ) : (
                <ShoppingCart size={16} style={{ color: "#c084fc" }} />
              )}
            </div>
            <div>
              <h1
                className="text-base font-heading font-bold"
                style={{ color: isDark ? "#f3e8ff" : "#581c87" }}
              >
                {title}
              </h1>
              {subtitle && (
...
/** Glass-styled stat card for Sales pages */
export const SalesGlassCard: React.FC<{
  label: string;
  ...
  return (
    <motion.div
      ...
      style={{
        background: isDark ? `rgba(17,11,26,0.5)` : `rgba(255,255,255,0.75)`,
