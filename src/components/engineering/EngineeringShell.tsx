...
/**
 * Shared glass-themed wrapper for Engineering module pages — mirrors
 * MaterialShell/FinanceShell/AdminShell/DbaShell so Engineering gets the
 * same ambient glow, frosted header band, and consistent layout as every
 * other module, tinted to the engineering accent (#f97316, matching
 * AppSidebar's MODULE_HEADER for "engineering").
 */
export const EngineeringShell: React.FC<EngineeringShellProps> = ({
  title,
  subtitle,
  icon: PageIcon,
  action,
  ...
  const glassCard = isDark
    ...
    : {
        ...
        boxShadow:
          "0 8px 32px rgba(249,115,22,0.08), inset 0 1px 0 rgba(255,255,255,0.9)",
      };
    ...
    <div className="relative min-h-full p-4 space-y-5">
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to bottom, transparent 10%, #f97316 30%, #f97316 70%, transparent 90%)",
          }}
        />
        ...
        <div className="relative z-10 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
          <div className="flex items-center gap-3">
            ...
            >
              {PageIcon ? (
                <PageIcon size={16} style={{ color: "#fb923c" }} />
              ) : (
            ...
            <div>
              <h1
                ...
                style={{ color: isDark ? "#fed7aa" : "#7c2d12" }}
              >
                {title}
              </h1>
              {subtitle && (
